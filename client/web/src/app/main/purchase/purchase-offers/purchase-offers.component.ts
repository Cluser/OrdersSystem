...
export class PurchaseOffersComponent implements OnInit {
  ...
  @ViewChild(PurchaseOffersSearchComponent)
  private purchaseOffersSearchComponent = {} as PurchaseOffersSearchComponent;
  ...
  public setFilterFromUrl(): void {
    ...
    this.filter.idDistributor = this.route.snapshot.queryParamMap
      .getAll('idDistributor')
      .map(Number);
    this.filter.idContactPerson = this.route.snapshot.queryParamMap
      .getAll('idContactPerson')
      .map(Number);
    this.filter.idUser = this.route.snapshot.queryParamMap
      .getAll('idUser')
      .map(Number);
    this.filter.archived = this.route.snapshot.queryParamMap
      .getAll('archived')
      .map((x) => x === 'true');
  }
  ...
  public getOffersData(): void {
    this.spinner.show();
    this.api.offer
      .getOffers(this.filter, 1, this.pageSize)
      .subscribe((response) => {
        this.grid = response;
    ...
    this.columnDefs = [
      ...
      },
      {
        ...
        valueFormatter: (params) =>
          currencyFormatter(params, params.data.items[0].currency),
        type: 'rightAligned',
      },
    ];
  }
  ...
  private calculateOffersPrices(offers: any): void {
    offers.forEach(
      (offer: any) =>
        (offer.totalPrice = Object.values(offer.items).reduce(
  ...
  public openAddOfferModal(): void {
    ...
    modalRef.componentInstance.offer.idDistributor =
      this.selectedRows[0].idDistributor;
    modalRef.componentInstance.offer.idContactPerson =
      this.selectedRows[0].idContactPerson;
    modalRef.componentInstance.items = this.selectedRows.flatMap(
      (rows) => (rows = rows.items)
    );
    modalRef.componentInstance.offerAddedEvent.subscribe(() =>
      this.router.navigate(['/main/purchase/offers'])
    );
  ...
  public openAddOrderModal(): void {
    ...
    modalRef.componentInstance.order.idDistributor =
      this.selectedRows[0].idDistributor;
    modalRef.componentInstance.order.idContactPerson =
      this.selectedRows[0].idContactPerson;
    modalRef.componentInstance.items = this.selectedRows.flatMap(
      (rows) => (rows = rows.items)
    );
    modalRef.componentInstance.orderAddedEvent.subscribe(() =>
      this.router.navigate(['/main/purchase/orders'])
    );
    modalRef.componentInstance.closeEvent.subscribe(() =>
      this.modalService.dismissAll()
    );
  ...
  public openEditOfferModal(offer: IOffer): void {
    const modalRef = this.modalService.open(PurchaseModalEditOfferComponent, {
      size: 'xl',
    });

    modalRef.componentInstance.offer = offer;
    modalRef.componentInstance.offerEdditedEvent.subscribe(() =>
      this.getOffersData()
    );
    modalRef.componentInstance.closeEvent.subscribe(() =>
      this.modalService.dismissAll()
  ...
  public openArchiveModal(): void {
    ...
    modalRef.componentInstance.archiveSelected.subscribe(() =>
