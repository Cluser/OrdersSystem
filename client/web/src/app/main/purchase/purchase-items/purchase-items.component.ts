...
export class PurchaseItemsComponent implements OnInit {
  ...
  public setFilterFromUrl(): void {
    ...
    this.filter.name = this.route.snapshot.queryParamMap
      .getAll('name')
      .map(String);
    this.filter.model = this.route.snapshot.queryParamMap
      .getAll('model')
      .map(String);
    this.filter.quantity = this.route.snapshot.queryParamMap
      .getAll('quantity')
      .map(Number);
    this.filter.idProject = this.route.snapshot.queryParamMap
      .getAll('idProject')
      .map(Number);
    this.filter.idCategory = this.route.snapshot.queryParamMap
      .getAll('idCategory')
      .map(Number);
    this.filter.archived = this.route.snapshot.queryParamMap
      .getAll('archived')
      .map((x) => x === 'true');
    this.filter.status = this.route.snapshot.queryParamMap
      .getAll('status')
      .map(String);
  }
  ...
  public getItemsData(): void {
    this.spinner.show();
    this.api.item
      .getItems(this.filter, '', '', 1, this.pageSize)
      .subscribe((response) => {
  ...
  public openAddItemModal(): void {
    ...
    modalRef.componentInstance.itemAddedEvent.subscribe(() =>
      this.getItemsData()
    );
  ...
  public openImportItemsModal(): void {
    ...
    modalRef.componentInstance.itemsImportedEvent.subscribe(() =>
      this.getItemsData()
    );
    modalRef.componentInstance.closeEvent.subscribe(() =>
      this.modalService.dismissAll()
  ...
  public openAddInquiryModal(): void {
    ...
    modalRef.componentInstance.inquiryAddedEvent.subscribe(() =>
      this.router.navigate(['/main/purchase/inquiries'])
    );
    modalRef.componentInstance.closeEvent.subscribe(() =>
      this.modalService.dismissAll()
    );
  ...
  public openEditItemModal(item: IItem): void {
    ...
    modalRef.componentInstance.itemEditedEvent.subscribe(() =>
      this.getItemsData()
    );
    modalRef.componentInstance.closeEvent.subscribe(() =>
      this.modalService.dismissAll()
