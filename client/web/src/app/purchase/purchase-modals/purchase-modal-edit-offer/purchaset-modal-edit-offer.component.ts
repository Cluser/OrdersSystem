...
import { IContactPerson, IDistributor, IOffer } from '../../../shared/models';
...
export class PurchaseModalEditOfferComponent implements OnInit {
  ...
  public prepareGrid(): void {
    this.columnDefs = [
      ...
      { field: 'price', headerName: 'Cena', sortable: true, filter: true, resizable: true, flex: 3, editable: true, valueFormatter: params => currencyFormatter(params, this.rowData[0].currency), type: 'rightAligned' },
      { field: 'total',  headerName: 'Razem', sortable: true, valueGetter: 'getValue("price") * getValue("quantity")', valueFormatter: params => currencyFormatter(params, this.rowData[0].currency), type: 'rightAligned' },
    ];
